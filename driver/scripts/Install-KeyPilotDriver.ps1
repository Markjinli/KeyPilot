...
param(
    ...
    [switch]$IUnderstandThisInstallsAKernelDriver
)
...
if (-not $IUnderstandThisInstallsAKernelDriver) {
    throw 'Installation was not run. Re-run with -IUnderstandThisInstallsAKernelDriver only after reviewing the recovery procedure.'
}
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    [Security.Principal.WindowsBuiltInRole]::Administrator)
if (-not $isAdmin) {
    throw 'Run this driver installation operation in an elevated PowerShell window.'
}
...
try {
    ...
    # Never execute a helper from the user-writable release directory after elevation.
    # Copy companions into the protected staging directory and pin their exact content to
    # hashes embedded in this already-parsed installer before invoking either one.
    $checkScript = Join-Path $stagedPackage 'Test-InstallPrerequisites.ps1'
    $recoveryScript = Join-Path $stagedPackage 'Recover-KeyPilotDriver.cmd'
    [IO.File]::Copy((Join-Path $PSScriptRoot 'Test-InstallPrerequisites.ps1'), $checkScript, $false)
    [IO.File]::Copy((Join-Path $PSScriptRoot 'Recover-KeyPilotDriver.cmd'), $recoveryScript, $false)
    $expectedCheckHash = 'B1D35002DD579FB0F98C64C972A9189B3F7DF858ECD715344EE6CEBE215D8DA9'
    ...
    if (-not [string]::Equals($actualCheckHash, $expectedCheckHash, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        throw 'A pinned driver-installation companion changed during staging. Nothing was installed.'
    }

    # Validate the immutable administrator-only copy, then pass that exact path to PnPUtil.
...
$recordPath = Join-Path $recordRoot 'installation-record.json'

function Write-RecoveryRecord([object]$Value) {
    $temporary = Join-Path $recordRoot ('.installation-record.{0}.tmp' -f [Guid]::NewGuid().ToString('N'))
...
# Capture the verified Driver Store set before mutation. Only a unique new
# published name may be recorded as the package installed by this run.
...
# Refuse to mutate PnP state unless a durable pending recovery record exists.
Write-RecoveryRecord $pendingRecord

Write-Warning 'A bad keyboard filter can disable keyboard input. Keep another input device and the recovery command available.'
...
if ($pnpUtilExitCode -notin @(0, 3010)) {
    $pendingRecord.Status = 'PnPUtilFailed'
    Write-RecoveryRecord $pendingRecord
...
try {
    ...
    $pendingRecord.Status = 'Installed-VerificationFailed'
...
if ($newMatching.Count -ne 1) {
    $pendingRecord.Status = 'Installed-PublishedNameAmbiguous'
    $pendingRecord['AllVerifiedPublishedNames'] = @($matching.Driver)
    $pendingRecord['NewVerifiedPublishedNames'] = @($newMatching.Driver)
    $pendingRecord['InstalledAtUtc'] = [DateTime]::UtcNow.ToString('O')
    Write-RecoveryRecord $pendingRecord
    throw "PnPUtil succeeded, but exactly one new verified KeyPilot package was not found. Do not guess an oemNN.inf; inspect Get-WindowsDriver -Online. Record: $recordPath"
}

$installedRecord = [ordered]@{}
foreach ($entry in $pendingRecord.GetEnumerator()) { $installedRecord[$entry.Key] = $entry.Value }
$installedRecord.Status = 'Installed'
...
Write-Host "Installation request completed. Recovery information: $recordPath"
if ($rebootRequired) {
    Write-Warning 'Windows accepted the package and reported that a reboot is required.'
}
} finally {
